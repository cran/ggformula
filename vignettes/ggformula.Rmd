...
Attributes can be specified in 4 ways:

 * adding an argument of the form `attribute = value` [set]
 * adding an argument of the form `attribute = ~ expression` [map]
 * adding a formula term of the form `+ attribute:value` to the formula [auto chooses set or map]
 * adding a formula term of the form `+ attribute::value` [map]
 
where `attribute` is one of `color`, `shape`, etc. and `value` is either a constant 
(e.g. `"red"` or `0.5`, as appropriate), a variable (e.g. `cyl`), and `expression`
...
When using the `:` separator, mapping will be used if `value` is the name of 
a variable in `data`, otherwise setting will be used.  The other forms 
can be used to make things explicit when this auto-detection method doesn't yield the 
desired results.  In general, the use of function arguments is preferable over adding additional
terms to the formula.
...
difference to make it easier to distingish the groups.
...
These stats use a function to aggegrate over unique values of `x` or over 
...
The other two values computed by `mean_se()` are availble as
`..ymin..` and `..ymax..`.
